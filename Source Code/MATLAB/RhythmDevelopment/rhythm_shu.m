...
pacingTraceTog = uicontrol('Parent',p1,'Style','checkbox','FontSize',12,'String','Pacing Trace','Position',[710 9 100 23],'Callback',{@pacingTraceTog_callback});
...
apply_button = uicontrol('Parent',cond_sig,'Style','pushbutton','FontSize',12,'String','Apply','Position',[3 2 96 30],'Callback',{@cond_sig_selcbk});
%Pop-up menu options
...
export_button = uicontrol('Parent',cond_sig,'Style','pushbutton','FontSize',12,'String','Export Data','Position',[209 2 96 30],'Callback',{@export_callback});
remove_button = uicontrol('Parent',cond_sig,'Style','pushbutton','FontSize',12,'String','Remove','Position',[106 2 96 30],'Callback',{@remove_callback});
...
set([f,p1,filelist,selectdir,refreshdir,loadfile,movie_scrn,movie_slider, signal_scrn1,signal_scrn2,signal_scrn3,...
    ...
    starttimesig_text,starttimesig_edit,endtimesig_text,endtimesig_edit,pacingTraceTog,remove_button],'Units','normalized')
...
set([removeBG_button,bg_thresh_edit,bg_thresh_label,perc_ex_edit,perc_ex_label,bin_button,filt_button,removeDrift_button,norm_button,...
    ...
    refreshdir,invert_cmap,export_button,pacingTraceTog,remove_button],'Enable','off')
    ...
    function loadfile_callback(~,~)
        if isempty(handles.filename)
            ...
            if isfield(Data,'cmosData2')
                ...
                handles.cmosData = double(Data.cmosData(:,:,2:end));
% % %                 % Load from single camera
% % %                 handles.cmosData = double(Data.cmosData(:,:,2:1000));
                handles.bg = double(Data.bgimage);
            ...
            set([removeBG_button,bg_thresh_edit,bg_thresh_label,perc_ex_edit,...
                ...
                endtimesig_edit,expwave_button,export_button,remove_button],'Enable','on')
            % Disable pacing trace toggle
            set(pacingTraceTog,'Enable','off','Value',0)            
        end
    end
...
%% Select directory for optical files
    function selectdir_callback(~,~)
        ...
        if dir_name ~= 0
            ...
            search_nameGSD = [dir_name,'/*.gsd'];
            files = struct2cell(dir(search_name));
            filesNew = struct2cell(dir(search_nameNew));
            filesGSD = struct2cell(dir(search_nameGSD));
            handles.file_list = [files(1,:)'; filesNew(1,:)'; filesGSD(1,:)'];
...
% %         
        ...
        if get(pacingTraceTog,'Value') == 1
            % Place a hold on the current signal axes
            tmp = sprintf('hold(signal_scrn%d,''on'')',wave_window);
            eval(tmp)
            % Overlay this axes with the pacing spike
            tmp = sprintf('plot(handles.pacingTime,handles.pacedSignal,''Color'',[169/255 169/255 169/255],''LineWidth'',2,''Parent'',signal_scrn%d)',wave_window);
            eval(tmp)
            % Place a hold on the current signal axes
            tmp = sprintf('hold(signal_scrn%d,''off'')',wave_window);
            eval(tmp)
            if handles.grabbed < 5
                tmp = sprintf('set(signal_scrn%d,''XTick'',[])',wave_window);
                eval(tmp)
            end
        end
...
%Construct a VideoWriter object and view its properties. Set the frame rate to 60 frames per second:
    function expmov_button_callback(~,~)        
        ...
        fin = round(str2double(get(endtimesig_edit,'String'))*handles.Fs+1);
...
%% Condition Signals Selection Change Callback
    function cond_sig_selcbk(~,~)
        ...
        if norm_state == 1
            ...
            set(pacingTraceTog,'Enable','on')
        else
            % Make pacing trace overlay button value 0
            set(pacingTraceTog,'Value',0)
            % Make pacing trace unavailable
            set(pacingTraceTog,'Enable','off')
        end
        % Remove atria if is mask exists
        if ~isempty(handles.removeMask)
            % Remove the selected points from the data matrix
            cmosData(repmat(handles.removeMask,[1 1 ...
                size(cmosData,3)])) = nan;
        end
...
% % %         coordinate=getrect(movie_scrn);
        ...
        M = [40 20;
            ...
            40 48;
            62 48];
        coordinate = [M(1,1) M(1,2) M(2,1)-M(1,1) M(3,2)-M(2,2)];
...
%% Callback for the button used to remove unwanted pixels
    function remove_callback(~,~)
        % Use a region of interest (ROI) polyline tool to select region
        BW = roipoly;
        % Save the mask
        if isempty(handles.removeMask)
            handles.removeMask = BW;
        else
            handles.removeMask = (handles.removeMask+BW) ~= 0;
        end
        % Remove the selected points from the data matrix
        handles.cmosData(repmat(BW,[1 1 size(handles.cmosData,3)])) = nan;
    end

%% Callback for overlaying the pacing spike onto the data %%
    function pacingTraceTog_callback(~,~)
        ...
        if get(pacingTraceTog,'Value') == 1
            ...
            for n = 1:size(M,1)
                ...
                tmp = sprintf('plot(handles.time,squeeze(handles.cmosData(M(n,2),M(n,1),:)),colax(n),''LineWidth'',2,''Parent'',signal_scrn%d)',n);
                eval(tmp)
