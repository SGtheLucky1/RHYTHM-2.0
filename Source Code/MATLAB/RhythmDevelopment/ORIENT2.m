...
%
% Author: Christopher Gloschat, Rayhaan Rasheed - GWU - Efimov Lab
% Date: October 12, 2016
%
% Modification Log:
%
% For any questions and suggestions, please email us at:
% cgloschat@gmail.com or igor@gwu.edu
%
...
%% Select directory for optical files
    function selectdir_callback(~,~)
        ...
        if dir_name ~= 0
            ...
            type = type(ia,:);
            %             ending = zeros(length(type),3);
            %             for n = 1:size(ending,1)
            %
            %             end
        ...
        if isempty(handles.currentfile)
            ...
            if loadflag ~= 0
                ...
                for n = 1:4
                    ...
                    % Load cmos data
                    % % %                 handles.cmosData{n} = double(Data{n}.cmosData(:,:,2:end));
                    % handles.cmosData{n} = double(Data{n}.cmosData(:,:,2:500));
...
%% Screen update
    function screenUpdate(data,dataGeo,frame)
        ...
        if ~strcmp(get(align_button,'String'),'ALIGN')
            ...
            % % %             M = handles.M{ac};[a,~]=size(M); colax='bgmky';
            % % %             hold on
            % % %             for x=1:a
            % % %                 scatter3(M(x,1),M(x,2),M(x,3),colax(x),'filled','MarkerEdgeColor','k','LineWidth',2,'SizeData',48,'Parent',camG_scrn)
            % % %             end
        end
...
%Construct a VideoWriter object and view its properties. Set the frame rate to 60 frames per second:
    function expmov_button_callback(~,~)
        ...
        for i = start:step:fin
            % % % %             % Rotate during third beat
            % % % %             if i >= 686
            % % % %                 % Update the azimuth value
            % % % %                handles.az = handles.az+rotStep;
            % % % %                % Update the text in the azimuth control box
...
%% Export signal waves to new screen
    function expwave_button_callback(~,~)
        ...
        axis vis3d
        view(az2,el)
        % % %         M = handles.M{handles.frame}; colax='bgmky'; [a,~]=size(M);
        % % %         if isempty(M)
        ...
        % % %                 plot(handles.time,squeeze(handles.cmosData{handles.frame}(M(x,2),M(x,1),:)),'color',colax(x),'LineWidth',2)
...
%% Condition Signals on Click of Apply Button
    function cond_sig_callback(~,~)
        ...
        for n = 1:4
            ...
            % % %             % SNR CALCULATION
            % % %             snr = aveSNR(cmosData,handles.ecg,handles.Fs,handles.nRate);
            % % %             handles.snr{n} = snr;
            % % %             snrInclude = snr > nanmean(nanmean(snr))-3*nanstd(nanstd(snr));
            % % %             tmp = repmat(snrInclude,[1 1 size(handles.cmosData{n},3)]);
            % % %             cmosData(~tmp) = nan;
...
%% Button to create analysis maps
    function createmap_button_callback(~,~)
        ...
        if check == 2
            ...
            % % %                 actMap,handles.Fs,handles.opt_dir,handles.currentfile);
            % % %             % Save to handles variable
            % % %             handles.actMapGeo = actMapGeo2;
        ...
        elseif check == 4
            ...
            % % %             nr = size(handles.geommask,1);
            % % %             nc = size(handles.geommask,2);
            % % %             [apdMapGeo,~,~,~,~] = dataProjection02(...
            % % %                 handles.newX,handles.newY,handles.shift,handles.norms,...
            % % %                 handles.centroids,handles.cells,handles.pts,...
            % % %                 handles.neighs,handles.neighnum,handles.Parmap,...
            % % %                 handles.Posmap,handles.mapCam,handles.bgMask,handles.bg,...
            % % %                 handles.geommask,nr,nc,handles.looknmap,...
            % % %                 apdMap,handles.Fs,handles.opt_dir,handles.currentfile);
            % Save to handles variable
...
%% Projection callback %%
    function project_callback(~,~)
        if strcmp(get(project,'String'),'PROJECT')
            ...
            % %             axes(camG_scrn)
            % %             trisurf(handles.cells,handles.pts(:,1),handles.pts(:,2),...
            % %                 handles.pts(:,3),handles.dataProj(:,215),'LineStyle','none')
            % %             set(camG_scrn,'XTick',[],'YTick',[],'ZTick',[])
            % %             colormap('jet')
