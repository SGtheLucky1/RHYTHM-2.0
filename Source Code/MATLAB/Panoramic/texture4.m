...
ii=[1 10 19 28 37 46 55 64];  % 0, 45, 90, 135, 180, 225, 270, 315
% Assign views to cells
disp('Assigning views to cells ...');
I=zeros(numnorms,1).*NaN;
for i=1:length(ii)
  gg=find(langle(:,ii(i))>135);
  J=find(isnan(I(gg)));
  I(gg(J))=ii(i);
end

txtyour=zeros(numnorms,length);
...
% Save the texture!
mediantxtyour=median(txtyour(:,1));
stdtxtyour=std(txtyour(:,1));
centroids(:,4)=txtyour(:,1);
