...
if fid~=-1
  ...
  bdir=input('images are stored in this directory (ie: /disk2/raw_data/map08_09_04_03/Geometry/scan11/ ): ','s');
  bfilename=input('basis of image filenames(ie: scan_ is basis for scan_000.pgm ): ','s');
  sfilename=input('suffix of image filenames(ie: pgm is suffix for scan_000.pgm ): ','s');
  ndigits=input('number of digits in image filenames(ie: 3 digits in cube001.bmp ): ');
  sdigit= input('start digit (ie: 0 or 1): ');
  [silh1,lims1,centroids1,orientations1,areas1,thresharr]=get_silhs(bdir,bfilename,sfilename,ndigits,sdigit,n_images);
end
if frontback
  go=1;
  while go
