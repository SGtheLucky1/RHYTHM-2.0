...
view([0 90])
for n = 1:4
    scatter3(xyzmap(n,1),xyzmap(n,2),xyzmap(n,3),'co')
    cmd = sprintf('text(xyzmap(n,1),xyzmap(n,2),xyzmap(n,3),''  Camera %s'',''Color'',''k'')',scanlabel(n));
    eval(cmd)
end
hold off
...
for n = 1:4
    ...
    ind = unique(ind);
    geomasks((n-1)*100^2+ind) = 1;
    geomasks(:,:,n) = imfill(geomasks(:,:,n))';
    figure(geoMap)
    subplot(2,2,n)
    imagesc(geomasks(:,:,n))
end
