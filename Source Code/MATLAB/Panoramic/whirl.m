...
if fid~=-1
  ...
  if notanalyzed==0
    ...
    fprintf('Found %d images that have not been analyzed.',notanalyzed);
...
notanalyzed=length(find(~analyzed));
if notanalyzed>0
  fprintf('%d images have not been analyzed.',notanalyzed);
...
while gofname
  ...
  fid=fopen(Rcfname);
  if fid==-1
    fprintf('Unable to open %s!',Rcfname);
  else
    fclose(fid);
    fprintf('Found %s',Rcfname);
    gofname=0;
  end
end
...
while gofname
  ...
  fid=fopen(Parfname);
  if fid==-1
    fprintf('Unable to open %s!',Parfname);
  else
    fclose(fid);
    fprintf('Found %s',Parfname);
    gofname=0;
  end
end
...
fprintf(' was calibrated? ie: camera calibrated cw and object scanned ccw? or');
fprintf('                     camera calibrated ccw and object scanned cw?');
fprintf('If so, then camera calibration parameters need to be re-ordered.');
reorder='g';
