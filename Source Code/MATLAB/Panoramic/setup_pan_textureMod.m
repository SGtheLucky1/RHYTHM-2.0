...
studydir = 'Rabbit_2016_0823';
geodir = 'Geometry';
caldir = 'Calibration';
vtkdir = 'VTK_Outputs';
geoName = 'xyzv_try1_povcyl_1mm_corrected_';
...
datafnames = cell(4,1);
for n = 1:4
    datafnames{n} = sprintf('%s%s/Analysis/%s_%s%s%s.mat',datadir,studydir,runnumber,runlabel,scanlabel(n),extralabel);
end
...
for n = 1:4
   mapcalfnames{n} = sprintf('%s%s/%s/%s/cal%s_%s.pospar',datadir,studydir,geodir,caldir,scanlabel(n),anum(n,:));
end
